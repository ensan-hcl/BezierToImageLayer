...
class Bezier2Image(torch.nn.Module):
    def __init__(self, n=30, w=60, length=160, alpha=0.0002, viewbox=(0, 0, 1.0, 1.0)):
        ...
        t = 2*torch.pow(t,3)-3*torch.square(t)+2*t  #y=2x^3-3x^2+2xに沿って密度を変更することで、より美しく描画ができる。この操作は計算量に影響しない。
        t_bar = 1 - t
        t_3_0 = torch.pow(t, 3)
        ...
        self.T = torch.stack([t_3_0, 3*t_2_1, 3*t_1_2, t_0_3], axis=1) #shape=(n, 4)

        board = torch.arange(0,self.w, dtype=torch.float32) / self.w                #shape=(w)
    ...
    def forward(self, x):
        ...
        reshaped_bezier = torch.reshape(x, (-1, x.shape[1], 4, 2))   #shape=(None, length, 4, 2)
        points = torch.matmul(self.T, reshaped_bezier)
        points = torch.reshape(points, (-1, self.length, self.n, 2))          #shape=(None, length, n, 2)
        ...
        result = torch.minimum(result, torch.ones_like(result))    #(画像フォーマットとしては必要)
...
model = Bezier2Image(length=4, w=50, n=60, alpha=0.0004, viewbox=(-1,-1,2,2))
y_pred = model(input)
