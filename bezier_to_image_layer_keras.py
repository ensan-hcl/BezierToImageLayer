...
class BezierToImageLayer(Layer):
    def __init__(self, n=30, w=60, length=160, alpha=0.0002, viewbox=(0, 0, 1.0, 1.0), **kwargs):
        ...
        t = 2*K.pow(t, 3)-3*K.square(t)+2*t
        t_bar = 1 - t
        t_3_0 = K.pow(t, 3)
        ...
        self.T = K.stack([t_3_0, 3*t_2_1, 3*t_1_2, t_0_3],
                         axis=1)  # shape=(n, 4)
        ...
        board = K.repeat_elements(
            board, self.length, axis=0)  # shape=(length, w, n)
    ...
    def build(self, input_shape):
        super(BezierToImageLayer, self).build(
    ...
    def call(self, x):
        ...
        # shape=(None, length, 4, 2)
        reshaped_bezier = K.reshape(x, (-1, x.shape[1], 4, 2))
        points = tf.matmul(self.T, reshaped_bezier)
        # shape=(None, length, n, 2)
        points = K.reshape(points, (-1, self.length, self.n, 2))
